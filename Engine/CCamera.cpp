...
void CCamera::update(RECT _rect)
{
	...
	if (input->getKeyboardState(DIK_W) & 0x80) m_position.z += movementspeed;
	if (input->getKeyboardState(DIK_S) & 0x80) m_position.z -= movementspeed;
	if (input->getKeyboardState(DIK_Q) & 0x80) m_position.y -= movementspeed;
	if (input->getKeyboardState(DIK_E) & 0x80) m_position.y += movementspeed;
